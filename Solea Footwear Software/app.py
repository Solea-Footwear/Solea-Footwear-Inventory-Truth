...
app = Flask(__name__)
CORS(app)
# R3-19: SECRET_KEY is required. Don't fall back to a guessable default.
...
def get_returns_by_brand():
    """R1-02: Return brand-level sold/return counts and return rate.

    Source data:
      sold_count    -- units sold on eBay, joined to products for brand
      return_count  -- MATCHED returns (internal_order_id NOT NULL) grouped by brand
      return_rate   -- return_count / sold_count, expressed as a percentage

    Optional query string filters:
      start_date, end_date (YYYY-MM-DD) -- inclusive, applied to opened_at for
                                          returns and sold_at for sold units.
    """
    db = next(get_db())
    try:
        from database import Return, Unit, Product
        from sqlalchemy import func

        start_date = request.args.get('start_date')
        end_date = request.args.get('end_date')

        # ---- sold counts per brand (eBay only) ----
        sold_q = db.query(
            Product.brand.label('brand'),
            func.count(Unit.id).label('sold_count')
        ).join(Product, Product.id == Unit.product_id) \
         .filter(Unit.sold_platform == 'eBay') \
         .filter(Unit.sold_at.isnot(None))
        if start_date:
            sold_q = sold_q.filter(Unit.sold_at >= start_date)
        if end_date:
            sold_q = sold_q.filter(Unit.sold_at <= end_date)
        sold_q = sold_q.group_by(Product.brand)
        sold_counts = {row.brand: row.sold_count for row in sold_q.all()}

        # ---- return counts per brand (matched returns only) ----
        ret_q = db.query(Return).filter(
            Return.brand.isnot(None),
            Return.internal_order_id.isnot(None),
        )
        if start_date:
            ret_q = ret_q.filter(Return.opened_at >= start_date)
        if end_date:
            ret_q = ret_q.filter(Return.opened_at <= end_date)
        returns = ret_q.all()

        brand_data = {}
        for r in returns:
            b = brand_data.setdefault(r.brand, {
                'return_count': 0,
            ...
            if r.final_outcome == 'refunded_after_return_received':
                b['refunded_after_return'] += 1
            elif r.final_outcome == 'refunded_without_return_received':
                b['refunded_without_return'] += 1
            elif r.final_outcome == 'closed_buyer_never_shipped':
                b['closed_buyer_never_shipped'] += 1

        # ---- Combine. Include brands present in either side. ----
        all_brands = set(sold_counts) | set(brand_data)
        rows = []
        for brand in all_brands:
            sold = sold_counts.get(brand, 0)
            data = brand_data.get(brand, {})
            ret_count = data.get('return_count', 0)
            return_rate = round(ret_count * 100.0 / sold, 2) if sold else None
            rows.append({
                'brand': brand,
                'sold_count': sold,
                'return_count': ret_count,
                'return_rate_percent': return_rate,
                'refunded_after_return': data.get('refunded_after_return', 0),
                'refunded_without_return': data.get('refunded_without_return', 0),
                'closed_buyer_never_shipped': data.get('closed_buyer_never_shipped', 0),
                # Backwards-compat keys for any existing dashboard code.
                'total_returns': ret_count,
                'percent_closed_buyer_never_shipped': (
                    round(data.get('closed_buyer_never_shipped', 0) * 100.0 / ret_count, 2)
                    if ret_count else 0
                ),
            })
        rows.sort(key=lambda x: (x['sold_count'] or 0, x['return_count']), reverse=True)
        return jsonify({'brands': rows})
    finally:
        db.close()
...
def manual_check_return_emails():
    ...
    try:
        ...
        # R3-22: gmail service paginates internally.
        emails = gmail.get_emails_from_label(label_name, max_results=1000)
        ...
        # R1-06: each email is its own commit boundary.
        for email_msg in emails:
            ...
            if email_processing.is_email_processed(message_id):
                ...
                continue
            try:
                parsed = parser.parse(email_msg)
                if not parsed:
                    email_processing.mark_email_processed(
                        message_id,
                        'failed',
                        f"parser returned None (subject={email_msg.get('subject', 'N/A')})",
                ...
                if result.get('success'):
                    email_processing.mark_email_processed(
                        ...
                        email_msg.get('subject'), email_msg.get('from'),
                    )
                    db.commit()
                    results['processed'] += 1
