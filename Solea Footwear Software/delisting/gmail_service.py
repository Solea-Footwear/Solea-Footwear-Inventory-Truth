...
import base64
import json
import pickle  # legacy - only used for one-time migration of old pickle tokens
...
class GmailService:
    ...
    def _initialize_gmail_api(self):
        ...
        R3-18: tokens are stored as JSON (Credentials.to_json /
        Credentials.from_authorized_user_info). Legacy pickle tokens are
        migrated to JSON on the next successful refresh.
        """
        try:
            json_path = os.getenv('GMAIL_TOKEN_PATH', 'gmail_token.json')
            legacy_pickle_path = 'gmail_token.pickle'

            # 1) Try JSON token first (the preferred path).
            if os.path.exists(json_path):
                with open(json_path, 'r') as fh:
                    info = json.load(fh)
                self.credentials = Credentials.from_authorized_user_info(info, SCOPES)

            # 2) Fall back to legacy pickle exactly once, then convert.
            elif os.path.exists(legacy_pickle_path):
                logger.info(
                    "Found legacy pickle token; migrating to JSON at %s", json_path,
                )
                with open(legacy_pickle_path, 'rb') as fh:
                    self.credentials = pickle.load(fh)
                self._save_credentials(json_path)
                try:
    ...
    def _save_credentials(self, path: str) -> None:
        """R3-18: write credentials as JSON."""
        try:
            with open(path, 'w') as fh:
                fh.write(self.credentials.to_json())
            try:
                os.chmod(path, 0o600)
            except OSError:
                pass
        except Exception as e:
            logger.error(f"Failed to persist Gmail credentials: {e}")
    ...
    def authenticate(self, credentials_path: str) -> bool:
        ...
        try:
            ...
            # R3-18: save credentials as JSON.
            json_path = os.getenv('GMAIL_TOKEN_PATH', 'gmail_token.json')
            self._save_credentials(json_path)
    ...
    def get_emails_from_label(self, label_name: str, max_results: int = 1000) -> List[Dict]:
        """R3-22: paginate through messages so labels with >100 messages
        are fully drained on a single call. A safety cap (max_results)
        prevents runaway loops; the default is 1000.

        Args:
            label_name: Name of the Gmail label.
            max_results: Hard upper bound on messages to fetch in one call.
        """
        if not self.service:
            logger.error("Gmail service not initialized")
            return []
        ...
        try:
            ...
            page_token = None
            page = 0
            PAGE_SIZE = 100  # Gmail max per page
            while True:
                page += 1
                resp = self.service.users().messages().list(
                    userId='me',
                    labelIds=[label_id],
                    maxResults=min(PAGE_SIZE, max_results - len(message_refs)),
                    pageToken=page_token,
                ).execute()
                batch = resp.get('messages', []) or []
                message_refs.extend(batch)
                logger.info(
                    f"Gmail label '{label_name}' page {page}: "
                    f"+{len(batch)} (total {len(message_refs)})"
                )
                page_token = resp.get('nextPageToken')
                if not page_token or len(message_refs) >= max_results:
                    break
            ...
            for ref in message_refs:
                try:
                    ...
                    logger.error(f"Gmail get() failed for {ref.get('id')}: {e}")

            logger.info(
                f"Retrieved {len(full_messages)} messages from label '{label_name}'"
            )
            return full_messages
