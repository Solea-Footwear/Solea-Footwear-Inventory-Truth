...
class eBayAPI:
    ...
    def __init__(self):
        ...
        OAuth 2.0 user tokens are sourced lazily from `ebay_token_store` on each
        API call; we do not read or cache them at construction time so that the
        application can boot even before consent has been granted.
        """
        ...
        if not all([self.app_id, self.cert_id, self.dev_id]):
            logger.warning("eBay API credentials (APP_ID/CERT_ID/DEV_ID) not fully configured")

    # ---- lazy Trading connection -------------------------------------------

    def _build_trading_connection(self):
        """Build a fresh `ebaysdk.trading.Connection` with a current OAuth access token."""
        from src.integrations.ebay import ebay_token_store
        if not all([self.app_id, self.cert_id, self.dev_id]):
            return None
        try:
            access_token = ebay_token_store.get_valid_access_token()
        except Exception as e:
            logger.warning(
                "No valid eBay OAuth access token available (%s). "
                "Visit /oauth/ebay/start to grant consent.", e
            )
            return None
        try:
            return Trading(
            ...
            logger.error("Failed to build eBay Trading connection: %s", e)
            return None

    @property
    def api(self):
        """Return a Trading connection seeded with a fresh OAuth access token."""
        return self._build_trading_connection()

    def is_configured(self):
        """True when app credentials are present and an OAuth token is on file."""
        if not all([self.app_id, self.cert_id, self.dev_id]):
            return False
        try:
            from src.integrations.ebay import ebay_token_store
            return ebay_token_store.has_tokens()
        except Exception:
            return False
    ...
    def get_return_details(self, return_id):
        ...
        """
        # No early return on EBAY_AUTH_TOKEN — the Post-Order API now uses OAuth
        # Bearer tokens via ebay_token_store.  If no tokens are available the
        # token store call below will raise and we'll log + return None.
        ...
        try:
            ...
            # OAuth 2.0 Bearer token (sourced via the encrypted token store with
            # automatic refresh on expiry — see src.integrations.ebay.ebay_token_store).
            from src.integrations.ebay import ebay_token_store
            headers = {
                'Authorization': f'Bearer {ebay_token_store.get_valid_access_token()}',
